...
namespace Pimcore\ImageTaggingBundle\Service;
...
class ImageTaggingService 
{
    ...
    private function linkAssetToTrainingLocation(Asset $asset, string $tagDir = null) {
        if ($tagDir == null) $tagDir = $this->cachePath . '/' . $asset->getFilename();
    ...
    private function getTagIdByName(string $name) {
        ...
        $params = array( 0 => $name );
        $types = array( 0 => 'string' );
        $result = $db->fetchAll($statement, $params, $types);
        if (sizeof($result) != 1) throw new \Exception('tagname not unique or not existing');
        return $result[0]["id"];
    }
    ...
    private function createLinks(Tag $child) {
        ...
        foreach ($pictures as $picture) {
            ...
            if ($assetPath != '') exec('ln ' . $assetPath  . ' ' . $tagDir);
        }
    }
