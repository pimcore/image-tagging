...
# Code added by Paul Hörmann

def preprocess_image(image, FLAGS):
    """Preprocesses image as defined by arguments.

    Takes all image supplied and applies preprocessing.
    In case no parameters are passed defaults are used.
    ...
    Returns:
      Preprocessed Image
    """
    tf.logging.info('preprocessing image')
    return image


def adapt_color(sess, image, FLAGS):
    """Adapts image color as defined by arguments.

    Takes image supplied and applies color transformation.
    In case no parameters are passed defaults are used.
    grayscale: transformation of rgb to grayscale image
    ...
    if FLAGS.color == 'grayscale':
        tf.logging.info('converting to grayscale')
        grayscaled_image = sess.run(tf.image.rgb_to_grayscale(image))
        tf.logging.info('converting back to rgb')
        # converting back needed because network needs three channels
        return sess.run(tf.image.grayscale_to_rgb(grayscaled_image))
    else:
        return image


def normalize_image(sess, image, FLAGS):
    """Normalizes image as defined by arguments.

    Takes image supplied and applies preprocessing and color adaption.
    In case no parameters are passed defaults are used.
    ...
    Returns:
      Normalized Image
    """
    tf.logging.info('normalizing image')
    preprocessed_image = preprocess_image(image, FLAGS)
    adapted_image = adapt_color(sess, preprocessed_image, FLAGS)
    tf.logging.info('normalizing done')
...
def create_module_graph(module_spec):
    ...
    tf.logging.info('expected image size: ' + str(height) + 'x' + str(width))
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
    ...
    grayscaled_vaules = normalize_image(sess, resized_input_values, FLAGS)
    bottleneck_values = sess.run(bottleneck_tensor,
                            {resized_input_tensor: grayscaled_vaules})
...
def main(_):
    ...
    start = time.time()
    tf.logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>')
    tf.logging.info('starting time: ' + str(start))
    tf.logging.info('<<<<<<<<<<<<<<<<<<<<<<<<<')
    ...
    with tf.Session(graph=graph, config=session_config) as sess:
        ...
        tf.logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>')
        tf.logging.info('end time: ' + str(end))
        tf.logging.info(', time needed: ' + str((end - start)))
        tf.logging.info('<<<<<<<<<<<<<<<<<<<<<<<<<')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='256x256',
        help="""\
      The resolution which should be used.\
      """)
    parser.add_argument(
        '--saved_model_dir',
        type=str,
